...
{
  // 라이다 - 아두이노
  lidar.begin(Serial1); // 라이다의 통신은 따로 pinmode를 사용하지않음 (하드웨어 시리얼)통신 사용 
                         // 메가의 Tx1, Rx1 포트에 꽂아서 통신이 되는지 확인해봐야함
...
// 파이 - 아두이노
  Serial.begin(9600); // 파이와의 통신속도는 9600이 좋은거같은데
                         ...
                         // 컴퓨터와 USB통신을 하려면 Tx, Rx핀을 다른 통신으로 사용할 수 없음
